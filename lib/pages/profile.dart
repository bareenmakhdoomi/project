...
class _ProfileState extends State<Profile> {
  ...
  void getPostsForUser(String userId) async {
    ...
    List<Object?> posts = documents.map((doc) => doc.data()).toList();

    print("Post check $posts");
  }
  ...
  void initState() {
    super.initState();
    getPostsForUser(firebaseAuth.currentUser!.uid);
    checkIfFollowing();
  }
  ...
  buildGridPost() {
    return StreamGridWrapper(
      ...
      itemBuilder: (_, DocumentSnapshot snapshot) {
        UserPost posts =
            UserPost.fromJson(snapshot.data() as Map<String, dynamic>);
        return PostTile(
          post: posts,
        );
      },
